...
async function modernizeHtml(response) {
  ...
  const marker = 'wa-center-v9.3-ui';
  if (html.includes(marker)) return new Response(html, {headers: response.headers, status: response.status});
  const scripts = UI_SCRIPTS.map(src => `<script src="${src}?v=9.3" defer></script>`).join('');
  const injection = `<!-- ${marker} --><link rel="stylesheet" href="${UI_CSS}?v=9.3">${scripts}<meta name="theme-color" content="#5f55f6"><meta name="apple-mobile-web-app-title" content="WA Center">`;
